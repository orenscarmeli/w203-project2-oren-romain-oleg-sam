...
We define brand awareness as the percent of a population that is aware of a brand and its product suite. We believe that brand awareness influences application success, rating, price, and category. Developers that have large backings and that spend money advertising their products are likely to be more successful than those who do not. Certain categories of applications, particularly those with broad audiences such as games, may be strongly affected by brand awareness. Applications released by popular brands may also have higher prices due to the power that comes with name recognition. 
\par
Brand awareness is possible to measure, but is not directly available in our data set. As noted above, we expect this omitted variable to be positively correlated with our independent and dependent variables, leading the bias away from zero and causing us to overestimate model coefficients.

\subsubsection{App Rankings}
Seasonality can influence consumer demand towards specific application categories. For example, fitness and health applications are likely to be more popular at the beginning of the year, when New Year's resolutions are at top of consumers' minds. To accurately model this variable, we would need to consider time series data rather than cross-sectional data. Although the expected impact of this omitted variable is weak, it is intimately tied to application category and could overemphasize (or underemphasize) the success of seasonally dependent applications.

\subsubsection{Differences in Consumer Bases}
This variable relates to the identity of a consumer base and how it engages with an application. Some consumers are readily targetable since applications fitting their profiles can be easily produced. However, consumers that demand specialized applications are more difficult to cater to. Some customer bases may also be substantially larger than others, acting as a massive body that funnels downloads for select applications. Additionally, application success may vary across demographics. Overall, this variable is multifaceted and difficult to measure, but may cause our conclusions about application success to be skewed in favor of large consumer bases.
