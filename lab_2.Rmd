...
```
From the regression results in Table 4, a few observations are of particular interest. First, across all models all coefficients are significant excluding two, `is_family_category` in Model 2, and `is_game_category` in Model 3. These variables were derived from the `category` variable in the raw dataset, which describes the category tagged for the application, i.e., Lifestyle, Game, Tools, etc. Additionally, these `category` variables are also the only coefficients which alternate around 0 across the different models, adding confusion to the direction of correlation with `log_installs`. 

Considering these factors, the research team has decided that Model 2 is the best choice for this exercise. All coefficients in Model 2 (except for `is_family_category`) have a p-value < 0.01, and although the explanatory power of Model 3 (${r^2} = 0.250$) is slightly better than Model 2 (${r^2} = 0.247$), Model 2 is easier to interpret and explain and has more significant and consistent coefficients. Both models show that `rating` has a positive influence on `log_installs`, with each 1 unit increase in rating corresponding to a ~15% increase in `log_installs`. These models generally agree with our hypothesis regarding the direction of correlation between each coefficient and the number of application installations. Finally, it is worthwhile to note that these models provide an ${r^2}$ value that would not be acceptable in fields of hard science (e.g., physics, molecular biology, etc.), it would potentially be acceptable in the field of social sciences to help understand a causal relationship. Given the extreme variability between types and end-use of applications available on the Google Play Store, as well as other uncontrolled factors, we believe that this model provides a useful description of how consumer rating may impact the success of an application. 
...
``` {r linear assumption plots, figures-side, fig.show = "hold", out.width = "50%", echo = FALSE}
res <- resid(model_medium)
plot(fitted(model_medium), res)
abline(0,0)
...
\par To verify the assumption of linear conditional expectations, we seek to show that there is no relationship between the model residuals and any of the predictor variables. That is, the model does not systematically underpredict or overpredict in certain regions of the input space. In the figure above, we see the relationship between the model residuals and metric-scale predictors. The residuals are generally well-centered around zero, although the model seems to underpredict when `log_reviews` is high and `rating` is low. The Q-Q plot shows the model residuals as a function of the model predictions. Here, the model seems to underpredict in the left-most and right-most regions, and slightly overpredict in the middle. Overall, we do generally see a linear relatoinship between the model residuals and the input features, so we do not find enough evidence to reject the assumption of linear conditional expectation.  

``` {r error plot, figures-side, fig.show = "hold", out.width = "50%", echo = FALSE}
plot(model_medium, which=3)
```
\subsubsection{Homoskedastic Errors}
\par When assessing homoskedastic errors, we seek to determine the relationship between the variance of the model residuals and the predictors. If the homoskedastic assumption is satisfied, then we should observe a lack of relationship (i.e., constant error variance across the plotted range); conversely, if the data are heteroskedastic then the conditional variance will depend on the predictors (i.e., non-constant error variance across the plotted range). The above plot is an eyeball test of homoskedasticity, showing the model residuals as a function of the model predictions. We notice that the spread of the residuals is mostly consistent throughout the x range, although the right-hand side is somewhat narrower. As a more concrete assessment, we also perform a Breush-Pagan test with the null hypothesis that there are no heteroskedastic errors in the model. Since the $p$-value falls below our significance threshold of 0.001, we find enough evidence to reject the null hypothesis. In response to this failed assumption, we report robust standard errors (adjusted for heteroskedasticity) instead of non-adjusted errors.

```{r error hist plot, figures-side, fig.show = "hold", out.width = "50%", echo = FALSE}
hist(res, main = "Normal Distribution of Model Residuals", breaks=20)
```
...
Earlier in this report, we hypothesized that higher consumer ratings will lead to greater application success, as described by the number of installations of an application on the Google Play store. The models produced from this analysis do agree with this hypothesis, and confirm that higher rated applications have more success on the store. Along with this conclusion, we found that free applications, newer applications, and larger (in terms of storage size) applications are also positive indicators for application installation. 
\par With regards to larger applications, this relationship is surprising because we would have two competing expectations where cellular data limits and phone storage limitations would results in larger apps that are less likely to be installed -or- users would pay little attention to application size and there would not be a relationship. Also surprisingly, newer updates are installed less, this could be a bias in the data or modeling methodology that we would have to revisit to further understand. It is worth considering that newly updated apps are inherently less likely to be installed by nature. Furthermore, game categorization is significant and increases installs in `Model 2` however it was not found to be significant in `Model 3`. `rating * is game category` was found to be significant in `Model 3` which could be hypothesized that games targeting a wider audience by being rated for everyone are more likely to be downloaded.
\par Though the explanatory power is low ($adj. {r^2} = 0.247$), it is sufficient enough to determine that a relationship exists. We believe further refining the modeling by including data from the discussed omitted variables would further improve the explanotory power of the model. 
