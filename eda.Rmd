...
calc_avg_rating_by_group <-
  function(column, quantile_table = FALSE) {
    ...
    result <- d_dt[, .(count_apps = .N,
                   ...
                   by = column][count_apps > 100][order(-rating_avg)]
...
table_long_cat <- rbindlist(lapply(categorical_cols, calc_avg_rating_by_group))
table_quantile_cat <- rbindlist(lapply(categorical_cols, calc_avg_rating_by_group,quantile_table=TRUE))
...
cols <- c('installs_clean', 'log_installs', 'log_reviews', 'log_size')
