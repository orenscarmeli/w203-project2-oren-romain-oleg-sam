...
calc_stats_by_group <-
  function(column, quantile_table = FALSE) {
    ...
    result <- d_dt[, .(count_apps = .N,
                       install_group_mean = mean(install_group, na.rm = TRUE),
                       install_count_median = median(installs_clean, na.rm = TRUE)), 
                   by = column][count_apps > 100][order(-install_group_mean)]
