...
calc_avg_rating_by_group <-
  function(column, quantile_table = FALSE) {
    ...
    result <- d_dt[, .(count_apps = .N,
                       rating_avg = mean(Rating, na.rm = TRUE)), by = column][count_apps > 100][order(-rating_avg)]
...
table_long <- rbindlist(lapply(categorical_cols, calc_avg_rating_by_group))
table_quantile <- rbindlist(lapply(categorical_cols, calc_avg_rating_by_group,quantile_table=TRUE))

# compare avg rating by group
table_long

# compare distribution of avg rating across groups
table_quantile
```
